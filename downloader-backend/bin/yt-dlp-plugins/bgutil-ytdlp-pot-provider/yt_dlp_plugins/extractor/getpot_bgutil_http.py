...
class BgUtilHTTPPTP(BgUtilPTPBase):
    ...
    def _real_request_pot(
        ...
        # Upstream only binds webpo clients; android/ios PLAYER/SUBS tokens
        # bind to the video ID just like the webpo PLAYER/SUBS branches.
        content_binding = get_webpo_content_binding(request)[0]
        if content_binding is None and request.innertube_context:
            client_name = traverse_obj(request.innertube_context, ('client', 'clientName'))
            if client_name in ('ANDROID', 'IOS') and request.context in (
                    PoTokenContext.PLAYER, PoTokenContext.SUBS):
                content_binding = request.video_id
