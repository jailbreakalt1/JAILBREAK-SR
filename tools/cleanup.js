...
function cleanupOldFiles() {
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (!error.message.includes('ENOENT') && !error.message.includes('EBUSY')) {
          console.warn(chalk.yellowBright('  ⚠ WARN: ') + chalk.white(`Error processing file ${filePath}:`) + chalk.gray(` ${error.message}`));
        }
      }
    }
    ...
    if (deletedCount > 0) {
      ...
      console.log(chalk.gray('  ── ') + chalk.cyan('CLEANUP') + chalk.gray(' ── ') + chalk.white(`Deleted ${deletedCount} file(s)`) + chalk.gray(', freed ') + chalk.greenBright(`${sizeMB} MB`));
    }
  } catch (error) {
    // Don't crash on cleanup errors
    console.error(chalk.redBright('  ✖ ERROR: ') + chalk.white('Error during cleanup:') + chalk.gray(` ${error.message}`));
  }
}
...
function startCleanup() {
  ...
  console.log(chalk.cyan('  ⧈ ') + chalk.white('CLEANUP ENGINE: ') + chalk.yellowBright('STARTING...'));
  cleanupOldFiles();
  ...
  console.log(chalk.cyan('  ⧈ ') + chalk.white('CLEANUP ENGINE: ') + chalk.greenBright('ACTIVE') + chalk.gray(` (every ${CLEANUP_INTERVAL_MS / 1000 / 60} min)`));
}
...
function stopCleanup() {
  if (cleanupInterval) {
    ...
    console.log(chalk.cyan('  ⧈ ') + chalk.white('CLEANUP ENGINE: ') + chalk.redBright('STOPPED'));
  }
}
